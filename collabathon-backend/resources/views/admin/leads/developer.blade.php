...
<x-layouts.admin active="leads" :title="$developer->company_name" section="Manage">
    ...
    <div class="flex items-center gap-3.5 mb-6">
        ...
        <div class="min-w-0">
            ...
            <p class="text-[12.5px] text-ink-3 mt-0.5">
                Every project of theirs, ranked by how many channel partners have requested it.
            </p>
        </div>
    </div>
    ...
    <div x-data="{ open: false }" class="mb-5">
        <button type="button" @click="open = !open"
            ...
            <span class="flex items-center gap-2.5 min-w-0">
                ...
                <span class="text-left min-w-0">
                    ...
                    <span class="block text-[11.5px] text-ink-3 truncate">Trend and acceptance rate for this developer</span>
                </span>
            </span>
        ...
        <div x-show="open" x-cloak x-transition.opacity.duration.200ms class="grid grid-cols-1 xl:grid-cols-3 gap-4 mt-4">
            ...
            <x-panel title="Acceptance rate" subtitle="Requested → accepted, this developer" padded>
                <div class="pt-1">
    ...
    <x-data-table
        ...
        empty-description="This developer has no projects with that name.">

        <x-slot:filters>
            <x-filter-select name="project_type" :options="$projectTypes" placeholder="All types" />
        </x-slot:filters>
        ...
        @foreach($properties as $property)
            <tr class="hover:bg-canvas transition-colors cursor-pointer"
                x-on:click="window.location = @js(route('admin.leads.project', [$developer, $property]))">
                <td class="px-4 py-3">
                    <div class="flex items-center gap-2.5 min-w-0">
                        ...
                        @endif
                        <div class="min-w-0">
                            <a href="{{ route('admin.leads.project', [$developer, $property]) }}"
                               class="text-[13px] font-medium text-ink hover:underline truncate block">
                                {{ $property->name }}
                            </a>
                            <p class="text-[11.5px] text-ink-3 truncate">{{ $property->project_type }}</p>
                        </div>
                    </div>
                </td>
                ...
                <td class="px-4 py-3 text-right">
                    <span class="text-[13px] font-medium text-ink nums">{{ number_format((int) $property->requests_count) }}</span>
                </td>
                <td class="px-4 py-3 text-right hidden lg:table-cell">
                    <span class="text-[12.5px] text-success nums">{{ number_format((int) $property->accepted_count) }}</span>
                </td>
                <td class="px-4 py-3 text-right hidden lg:table-cell">
                    <span class="text-[12.5px] text-warning nums">{{ number_format((int) $property->pending_count) }}</span>
                </td>
                <td class="px-4 py-3 text-right hidden xl:table-cell">
                    <span class="text-[12.5px] text-danger nums">{{ number_format((int) $property->declined_count) }}</span>
                </td>
